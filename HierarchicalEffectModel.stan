...
transformed parameters{
  ...
  real MUE; 
    ...
    for(i in 1:PL)
      A_coef_hat[i]= rep_matrix(A_coef,N)+U[i];
...
model { 
 ...
 for(n in 1:N){
   ...
   OD[n,1:L]~normal(A_coef_hat[pl[n],,n]'*X[,1:L],sigma_o[1,pl[n]]);
   OD[n,(L+1):T]~normal(A_coef_hat[pl[n],,n]'*X[,(L+1):T],sigma_o[2,pl[n]]);
 }
