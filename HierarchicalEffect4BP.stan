  ...
  DA_coef=(1/MAXT)*(A_coef'*D)';
                    ...
                    MULAG=0+LAG;
                    {
                      ...
                      lpS_32[1]=0;
                       lpS_lag[1]=0;
                      for (t in 1:T){
                        lpS_lag[t+1]=lpS_lag[t]+normal_lpdf(loss[t]|MULAG,lambda[1]);
                        lpS_12[t+1]=lpS_12[t]+normal_lpdf(loss[t]|MUL,lambda[2]);
                        lpS_21[t+1]=lpS_21[t]+normal_lpdf(loss[t]|MUG,lambda[3]);
                        lpS_32[t+1]=lpS_32[t]+normal_lpdf(loss[t]|MUE,lambda[4]);
                      }
                      
                      for(s1 in 1:P)
                        for(s2 in (s1+1):P){
                          lp[s1,s2]= lp[s1,s2] + lpS_32[T+1] + lpS_lag[LAGT]+ (lpS_12[prior[s1]]-lpS_12[LAGT]) +(lpS_21[prior[s2]]-lpS_21[prior[s1]])  - lpS_32[prior[s2]];
                        }
                    }
    ...
    PR=P;
     ...
     for(i in 1:P){
      ...
      for (t in 1:T) {
         ...
         if(t<LAGT){
           piece_pred[t]= OD_pred[1]*exp(MULAG*MAXT*timeN[t]);
         }
        else{
          if(t<prior[s1]){
          piece_pred[t]= OD_pred[LAGT]*exp(MUL*MAXT*(timeN[t]-timeN[LAGT]));
