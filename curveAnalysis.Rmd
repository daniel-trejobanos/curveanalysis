...
prior<-which(p$posterior.prob>0)
...
Q<-subdivision.matrix(0.1,M)
I<-bernstein.integral.operator(M)
OD<-OD.old[1:N,]
#OD<-t(scale(t(OD)))
#MIN=min(log(OD))
#MAX=max(log(OD))
...
T <- length(timeN)
model <- stan_model(file = "BernsteinFitDynamic.stan")
rstan_options(auto_write = TRUE)
options(mc.cores = parallel::detectCores())
#fit<-stan("BernsteinFitDynamic.stan",chains=6,iter = 3000)
...
matplot((1/MAXT)*DODp/ODp,type="l",main="loss posterior")
