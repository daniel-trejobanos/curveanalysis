...
```{r}
library(readr)
SV <- read_csv("~/repo/curveanalysis/CSVFiles/CSV_for_ConditionsStandardConditions,YeastStrainYMR280C_Plate_24429.csv")
SV2 <- read_csv("~/repo/curveanalysis/CSVFiles/CSV_for_ConditionsStandardConditions,YeastStrainYMR280C_Plate_24426.csv")
SV3 <- read_csv("~/repo/curveanalysis/CSVFiles/CSV_for_ConditionsStandardConditions,YeastStrainYMR280C_Plate_24430.csv")
SV4 <- read_csv("~/repo/curveanalysis/CSVFiles/CSV_for_ConditionsStandardConditions,YeastStrainYMR280C_Plate_24428.csv")
current.data<-SV
current.data[grep("X",names(current.data))]<-NULL
current.data
OD.old<-as.matrix(current.data[,-1])
...
#T <- length(timeN)
#model <- stan_model(file = "BernsteinFitLOG.stan")
model <- stan_model(file = "HierarchicalEffectModel.stan")
rstan_options(auto_write = TRUE)
options(mc.cores = parallel::detectCores())
#fit<-vb(model)

#fit<-stan("BernsteinFitDynamic.stan",chains=2, seed=123)
fit<-stan("HierarchicalEffectModel.stan",iter=4000,chains = 4)
save(list="fit",file="fit.RData")
